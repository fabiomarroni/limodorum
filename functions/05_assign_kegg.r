...
option_list = list(
  make_option(c("-I", "--infile"), type="character", default="",help="Formatted DE+GO file [default= %default]", metavar="character"), 
  make_option(c("-O", "--outfile"), type="character", default="", help="output file (contains DE with GO and KEGG) [default= %default]", metavar="character")
)
...
indatacomma<-format(indata,decimal.mark=",")
write.table(indatacomma,gsub("2020.txt","2020_comma.txt",outfile),quote=F,row.names=F,sep="\t")
write_xlsx(indatacomma,gsub("2020.txt","2020.xls",outfile))
