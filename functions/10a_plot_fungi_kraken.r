...
option_list = list(
  ...
  make_option(c("-S", "--samplename"), type="character", default="LM",help="Sample [default= %default]", metavar="character"),
...
if (is.null(opt$samplename)) {
  stop("WARNING: No samplename specified with '-S' flag.")
} else {  cat ("samplename ", opt$samplename, "\n")
...
plot_species<-function(infile,taxid,samplename,outfile, level)
{
library(data.table)
...
ciccio<-grep(samplename,names(taxdat))
...
plot_species(infile=infile,taxid=taxid,samplename=samplename,outfile=outfile,level=level)
