...
option_list = list(
  ...
  make_option(c("-I", "--infile"), type="character", default="",help="DE file with annotation [default= %default]", metavar="character"), 
  make_option(c("-N", "--ontology"), type="character", default="BP", help="GO ontology (can be 'BP','CC' or 'MF') [default= %default]", metavar="character"),
  make_option(c("-O", "--outfile"), type="character", default="", help="GO enrichment results [default= %default]", metavar="character")
 )
...
GO.enrich<-function(GOfile,infile,outfile,ontology,top.res=50)
{
library(topGO)
library(data.table)
...
mytopgo<-new("topGOdata",ontology="BP",allGenes=fullset,annot = annFUN.gene2GO, geneSel= topgomerda , gene2GO = mygene2GO)
