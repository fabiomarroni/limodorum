...
option_list = list(
  ...
  make_option(c("-B", "--blastfile"), type="character", default="",help="Blastx of genes read from DE and associated with GO [default= %default]", metavar="character"), 
  make_option(c("-D", "--defile"), type="character", default="",help="DE result file modified to add phylum information [default= %default]", metavar="character"), 
  make_option(c("-S", "--sumfile"), type="character", default="",help="Summary phyla output file [default= %default]", metavar="character"),
  make_option(c("-O", "--out"), type="character", default="", help="output file (contains DE with GO) [default= %default]", metavar="character")
)
...
if (is.null(opt$sumfile)) {
  stop("WARNING: No Summary file specified with '-S' flag.")
} else {  cat ("Summary file is ", opt$sumfile, "\n")
  sumfile <- opt$sumfile  
  }
...
reformat.go<-function(gofile,blastfile,defile,sumfile,outfile,minfrac=0.01)
{
library(data.table)
...
write.table(DEsmallcomma,gsub("GO.txt","GO_comma.txt",outfile),quote=F,row.names=F,sep="\t")
mysumtab<-aggregate(DEsmall$Phylum,by=list(DEsmall$Phylum),FUN=length)		
setnames(mysumtab,c("Phylum","Count"))
total<-sum(mysumtab$Count)	
comptab<-mysumtab[mysumtab$Count>=minfrac*total,]	
comptab<-rbind(comptab,c("Others",total-sum(comptab$Count)))	
write.table(comptab,sumfile,sep="\t",row.names=F,quote=F)	

}
reformat.go(gofile=gofile,blastfile=blastfile,defile=defile,sumfile=sumfile,outfile=outfile)
